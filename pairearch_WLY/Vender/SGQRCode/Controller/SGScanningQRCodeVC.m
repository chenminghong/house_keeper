...
/** 扫描内容的Y值 */
#define scan_content_Y self.view.frame.size.height * 0.24
/** 扫描内容的X值 */
#define scan_content_X self.view.frame.size.width * 0.15
...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:(UIBarButtonItemStyleDone) target:self action:@selector(cancelBarButtonItemAction)];
}

//初始化扫描控制器
+ (SGScanningQRCodeVC *)getSgscanningQRCodeVCWithResultBlock:(ScanQrcodeResultBlock)resultBlock {
    SGScanningQRCodeVC *scanningVC = [SGScanningQRCodeVC new];
    scanningVC.scanResultBlock = resultBlock;
    return scanningVC;
}
...
- (void)cancelBarButtonItemAction {
    ...
    self.scanResultBlock(@"click_cancel");
}
...
- (void)scanQRCodeFromPhotosInTheAlbum:(UIImage *)image {
    ...
    if (features.count >= 1) {
        ...
        self.alertView = [SGAlertView alertViewWithTitle:nil delegate:nil contentTitle:@"未发现二维码/条形码" alertViewBottomViewType:(SGAlertViewBottomViewTypeOne)];
        [self.alertView show];
    }
}
